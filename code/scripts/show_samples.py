...
parser.add_argument('output_folder', type=str, nargs='?', default='cropped/positive', help='Output folder for cropped and resised image windows')
...
with open(args.info_dat, 'r') as dat_file:
	for line in random.sample(dat_file.readlines(), 400):
...
for image_path, details in info.iteritems():
	if os.path.isfile(image_path):
		...
		thumb_out_path = '{}/{}.thumbnail.jpg'.format(args.output_folder, file_name)
		original_out_path = '{}/{}.jpg'.format(args.output_folder, file_name)
		...
		try:
			...
			for i in range(1, 4 * num_objects, 4):
				...
				im_crop = im_crop.resize(size, Image.ANTIALIAS).convert('L')
				im_crop.save(thumb_out_path, 'jpeg')
				im.save(original_out_path, 'jpeg')
		except IOError:
