...
\begin{document}
    ...
    \label{sec:plan}
		...
		We will train and test our sphere detector on a purpose built image
        database, the creation of which is outlined in
        ...
        required for detection, and the training time required. Time
        permitting, we will also compare the suitability of the best
        techniques for online detection by running them on a robot.

        To ensure the timely completion of the project, existing libraries
        will be utilised whenever possible in the implementation of the chosen
        classification techniques. The NUbots' existing codebase will be
        considered as a potential starting point for the development of the
        new classifiers and test platform.
